...
To solve problems open one of the example projects. In each level you have to move each Kara to its own Cloverleaf and collect it with collect Cloverleaf. Aside from trees there are different challenges with additional components: some directional fields, marked with arrows, can only be entered from these directions; other fields are marked with stripes and only one Kara can be in this area at a time. When one Kara is inside the area, the stripes turn from green to red, indicating no additional Kara can enter. After completing a level you can progress to the next with the continue Button.
...
If you have moved Kara in a tricky spot you can always change its position in the grid via the Squeak Halo. Additionally you can change the color of the Kara with the colorpicker.
...
The LandingPage opens the Environment, which consists of the Grid, Workspace and the Executer. All different Components on the grid are children of a central GridObject class.
